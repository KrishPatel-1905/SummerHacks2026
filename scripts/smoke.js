...
try {
  ...
  ({ event, ownerToken } = await json(await fetch(`${origin}/api/events`, {
    method: "POST",
  ...
  const joined = await json(await fetch(`${origin}/api/events/join/${event.inviteCode}`));
  ...
  const memory = await json(await fetch(`${origin}/api/events/${event.id}/memories`, {
    method: "POST",
  ...
  const pulse = await json(await fetch(`${origin}/api/events/${event.id}/pulse`, {
    headers: { "x-capsule-code": event.inviteCode },
  }));
  assert.equal(pulse.pulse.analysisCoverage, 100);
  assert.equal(pulse.pulse.memoryCount, 1);

  const closed = await json(await fetch(`${origin}/api/events/${event.id}`, {
    method: "PATCH",
  ...
  if (event && ownerToken) {
    const cleanup = await fetch(`${origin}/api/events/${event.id}`, {
      method: "DELETE",
